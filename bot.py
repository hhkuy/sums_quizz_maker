...
def generate_subtopics_inline_keyboard(topic, topic_index):
    ...
    for j, sub in enumerate(subtopics):
        btn = InlineKeyboardButton(
            text=f"{sub['name']} ({len(fetch_questions(sub['file']))})",
...
# زرار ارسال كل الاسئلة الموجودة في الموضوع الفرعي
async def generate_questions_options(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    send_all_btn = InlineKeyboardButton("إرسال كل الأسئلة", callback_data="send_all_questions")
    set_number_btn = InlineKeyboardButton("تحديد عدد الأسئلة", callback_data="set_number_questions")
    back_btn = InlineKeyboardButton("« رجوع", callback_data=f"go_back_subtopics_{t_idx}")
    keyboard = InlineKeyboardMarkup([[send_all_btn], [set_number_btn], [back_btn]])

    await update.callback_query.message.edit_text(
        text="اختر طريقة الاختبار:",
...
async def callback_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # 3) اختيار موضوع فرعي
        ...
        await generate_questions_options(update,context)

    elif data == "send_all_questions":
        topics_data = context.user_data.get(TOPICS_KEY, [])
        t_idx = context.user_data.get(CUR_TOPIC_IDX_KEY, 0)
        s_idx = context.user_data.get(CUR_SUBTOPIC_IDX_KEY, 0)
        subtopics = topics_data[t_idx].get("subTopics", [])
        ...
        context.user_data[NUM_QUESTIONS_KEY] = len(questions)
        await send_questions(update,context,questions)

    elif data == "set_number_questions":
        t_idx = context.user_data.get(CUR_TOPIC_IDX_KEY, 0)
...
async def message_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if user_state == STATE_ASK_NUM_QUESTIONS:
        ...
        await send_questions(update,context,selected_questions)

    else:
        # أي رسالة أخرى لا نفعل بها شيئًا (ما لم تكن من ضمن التريجر في المجموعات)
        pass

async def send_questions(update,context,selected_questions):
    num_q = context.user_data[NUM_QUESTIONS_KEY]
    await update.message.reply_text(
    ...
    for idx, q in enumerate(selected_questions, start=1):
        ...
        correct_option_id = q.get("answer", 0)  # غالبًا 0-based
